"""
SegFormer-B2  --  Single-phase Training  [rnd: random 80/20 split]
Loss: 35% Weighted CrossEntropy + 65% Weighted Dice  (od 1. epochy)

Experiment: testuje či dvojfázové trénovanie prináša zlepšenie
            oproti jednofázovému tréningu s kombinovanou stratou.

Identické s train_segformer_phase1_rnd.py okrem loss funkcie:
  phase1_rnd : CrossEntropyLoss
  tento skript: 35% CE + 65% Dice  (rovnaké ako Phase 2)

Outputs:
  net_segformer_b2_single_rnd.pth     -- best model weights
  training_history_single_rnd.json    -- per-epoch metrics
"""
...
SAVE_PATH    = DATA_DIR / "net_segformer_b2_single_rnd.pth"
HISTORY_PATH = DATA_DIR / "training_history_single_rnd.json"
...
def main():
    ...
    print(f"Loss    : {CE_WEIGHT:.0%} CE + {DICE_WEIGHT:.0%} Dice  (od 1. epochy)")
    ...
    header = (f"{'Ep':>4} | {'TrLoss':>8} | {'VaLoss':>8} | {'mIoU':>7} | "
              ...
              + " | LR")
    print(f"\n{'='*len(header)}")
    print(f"  Single-phase rnd: {CE_WEIGHT:.0%} CE + {DICE_WEIGHT:.0%} Dice  |  LR={LR}")
    print(f"{'='*len(header)}")
    print(header)
    print(f"{'-'*len(header)}")
    ...
    print(f"\nSingle-phase rnd done. Best mIoU: {best_miou*100:.2f}%")
