"""
Test set evaluation -- RGB adjacent-slice variant.

Pre kazdu test snimku vytvori 3-kanalovy obrazok zo susednych rezov:
  R = (n-1), G = (n), B = (n+1)
"""
...
DEFAULT_MODEL = DATA_DIR / "net_segformer_b2_v3_ablation_rgb.pth"
...
def build_trunk_index(test_image_paths):
    ...
    print(f"  Pre-loading slices from {len(trunks)} trunks (pre RGB encoding)...",
          end=" ", flush=True)
    trunk_data = {}
    trunk_index = {}
    total = 0
...
def build_rgb_input(img_path, trunk_data, trunk_index):
    """Vytvori (H, W, 3) numpy uint8 RGB: R=prev, G=curr, B=next."""
    trunk = img_path.parent.parent.name
    stem  = img_path.stem
    ...
    return np.stack([s_prev, s_curr, s_next], axis=-1)
...
def main(model_path: Path, test_split: Path, output_suffix: str = ""):
    ...
    print(f"Mode       : RGB adjacent-slice  (R=n-1, G=n, B=n+1)")
    ...
    for img_path, msk_path in tqdm(zip(img_paths, msk_paths),
        ...
        rgb = build_rgb_input(img_path, trunk_data, trunk_index)

        out_t  = transform(image=rgb, mask=msk_gt)
    ...
    results["mode"]       = "RGB adjacent-slice (R=n-1, G=n, B=n+1)"
