"""
SegFormer-B2  --  Training v3 ABLATION
=======================================
Identicky s train_segformer_v3.py okrem:
  - nacita splits/ablation_{train,val}_files.json (bez Dub_praskliny_a a hrce_mixed)
  - vystupy: net_segformer_b2_v3_ablation.pth, training_history_v3_ablation.json

Ucel: zmerat o kolko prinieslo pridanie tych 114 dodatocnych snimok do trainu.
Test set ostava IDENTICKY (splits/test_files.json), takze finalne porovnanie
v3 vs v3_ablation je ciste ablation studium na rovnakom hodnotici sete.
"""
...
SAVE_PATH        = DATA_DIR / "net_segformer_b2_v3_ablation.pth"
HISTORY_PATH     = DATA_DIR / "training_history_v3_ablation.json"
...
def main():
    ...
    print(f"Splits  : {SPLITS_DIR} (ablation -- bez Dub_praskliny_a a hrce_mixed)")
    ...
    print("\nComputing class pixel frequencies (ablation train only)...")
    ...
    header = (f"{'Ep':>4} | {'TrLoss':>8} | {'VaLoss':>8} | {'mIoU':>7} | "
              ...
              + " | LR")
    print(f"\n{'='*len(header)}")
    print(f"  v3 ABLATION: {CE_WEIGHT:.0%} CE + {DICE_WEIGHT:.0%} Dice  |  LR={LR}  |  bez Dub_praskliny_a a hrce_mixed")
    print(f"{'='*len(header)}")
    print(header)
    print(f"{'-'*len(header)}")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        with open(HISTORY_PATH, "w") as f:
            json.dump({
                ...
                "split_source": "splits/ablation_{train,val}_files.json (bez Dub_praskliny_a a hrce_mixed)",
    ...
    print(f"\nv3 ablation done. Best mIoU: {best_miou*100:.2f}%")
