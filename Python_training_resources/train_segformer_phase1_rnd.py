"""
SegFormer-B2  --  Phase 1 Training  [rnd: random 80/20 split]

Identické hyperparametre ako train_segformer_phase1.py, jediný rozdiel:
  Split: náhodný 80/20 zo VŠETKÝCH snímok všetkých 20 kmeňov
         (rovnaká metodika ako starý projekt DeepLab_hrce)

Účel: porovnanie trunk-level vs random split -- výsledky nie sú priamo
      porovnateľné (random split má data leakage medzi train a val).

Outputs:
  net_segformer_b2_p1_rnd.pth     -- best model weights
  training_history_p1_rnd.json    -- per-epoch metrics
"""
...
NUM_CLASSES = 5
SEED        = 42
SPLIT_SEED  = 0      # rovnaký ako starý projekt (np.random.seed(0))
VAL_RATIO   = 0.20
...
SAVE_PATH    = DATA_DIR / "net_segformer_b2_p1_rnd.pth"
HISTORY_PATH = DATA_DIR / "training_history_p1_rnd.json"
...
def collect_all_pairs():
    """Collect ALL image/mask pairs from all 20 trunks."""
    imgs, msks = [], []
...
def random_split(imgs, msks, val_ratio, seed):
    """Random 80/20 split across all images (same method as old project)."""
    n = len(imgs)
    ...
    train_msks = [msks[i] for i in train_idx]
    val_imgs   = [imgs[i] for i in val_idx]
    val_msks   = [msks[i] for i in val_idx]
...
def main():
    ...
    print(f"Split   : random {int((1-VAL_RATIO)*100)}/{int(VAL_RATIO*100)}  (seed={SPLIT_SEED})")
    ...
    # Report which trunks appear in val (for transparency)
    val_trunks_seen = sorted({p.parent.parent.name for p in val_imgs})
    print(f"Val trunks ({len(val_trunks_seen)}): {', '.join(val_trunks_seen)}")
    ...
    header = (f"{'Ep':>4} | {'TrLoss':>8} | {'VaLoss':>8} | {'mIoU':>7} | "
              ...
              + " | LR")
    print(f"\n{'='*len(header)}")
    print(f"  Phase 1 rnd: random 80/20 split  |  LR={LR}")
    print(f"{'='*len(header)}")
    print(header)
    print(f"{'-'*len(header)}")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        with open(HISTORY_PATH, "w") as f:
            json.dump({
                ...
                "val_trunks_present": val_trunks_seen,
                "history": history,
            }, f, indent=2)
    ...
    print(f"\nPhase 1 rnd done. Best mIoU: {best_miou*100:.2f}%")
