...
Outputs:
  net_segformer_b2_v3p5.pth     -- best model weights
  training_history_v3p5.json    -- per-epoch metrics
"""
...
SAVE_PATH    = DATA_DIR / "net_segformer_b2_v3p5.pth"
HISTORY_PATH = DATA_DIR / "training_history_v3p5.json"
...
def main():
    ...
    # ── Load splits ──
    train_imgs, train_msks = load_split("train_v3p5")
    val_imgs,   val_msks   = load_split("val_v3p5")
    ...
    header = (f"{'Ep':>4} | {'TrLoss':>8} | {'VaLoss':>8} | {'mIoU':>7} | "
              ...
              + " | LR")
    print(f"\n{'='*len(header)}")
    print(f"  v3p5: {CE_WEIGHT:.0%} CE + {DICE_WEIGHT:.0%} Dice  |  LR={LR}  |  hybrid split")
    print(f"{'='*len(header)}")
    print(header)
    print(f"{'-'*len(header)}")
    ...
    print(f"\nv3p5 done. Best mIoU: {best_miou*100:.2f}%")
