"""
SegFormer-B2  --  Training v4 + Focal-Tversky Loss
====================================================
Identicke s train_segformer_v4.py okrem loss funkcie:
...
Outputs:
  net_segformer_b2_v4_focaltversky.pth
  training_history_v4_focaltversky.json
"""
...
SAVE_PATH    = DATA_DIR / "net_segformer_b2_v4_focaltversky.pth"
HISTORY_PATH = DATA_DIR / "training_history_v4_focaltversky.json"
...
def main():
    ...
    header = (f"{'Ep':>4} | {'TrLoss':>8} | {'VaLoss':>8} | {'mIoU':>7} | "
              ...
              + " | LR")
    print(f"\n{'='*len(header)}")
    print(f"  v4 + FOCAL-TVERSKY: {CE_WEIGHT:.0%} CE + {FT_WEIGHT:.0%} FT (gamma={GAMMA})")
    print(f"{'='*len(header)}")
    print(header)
    print(f"{'-'*len(header)}")
    ...
    print(f"\nv4 Focal-Tversky done. Best mIoU: {best_miou*100:.2f}%")
